...
function watchAssetsVender() {
    ...
    var taskConf = watchTaskConf[type],
        task;
    if (!taskConf) {
        watchTaskConf[type] = taskConf = genNewTaskConf(type, config.webappDir, config.dist);
    ...
    var moduleAssets = env.modules.map(function (m) {
        return config.webappDir + 'assets/pages/' + m.name + '/**/*';
    });
...
function watchDist() {
    ...
    var taskConf = watchTaskConf[type],
        task;
    if (!taskConf) {
        watchTaskConf[type] = taskConf = genNewTaskConf(type, config.dist, [
            // config.webTargetDir + 'classes/',
            config.webTargetDir + util.parseProjectName() + '-' + util.parseVersion() + '/'], true);
        task = new GenNewTask(taskConf);
    }
    return watch(config.dist + '**/*.*', WatchEvents, function (vinyl) {
        task.addToWatching({event: vinyl.event, path: vinyl.path, base: vinyl.base});
    });
...
function watchNormalJsHtml() {
    ...
    return watch(filesToWatch, WatchEvents, function (vinyl) {
        ...
        var distPath = vinyl.path.split(path.sep).join('/').replace(config.webappDir, config.tmp);
        fse.removeSync(distPath);
        if (vinyl.event === 'unlink') {
            // 已经处理
            return;
        }
        return gulp.src(vinyl.path, {base: config.webappDir})
