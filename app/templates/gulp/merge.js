...
function mergeCssByModule() {
    ...
    modules.forEach(function (m) {
        ...
        gulp.src([dir + '/*.css'])
            ...
            .pipe(gulp.dest(config.tmp + 'app/' + m.name))
            .on('end', function () {
                asyncCount--;
...
function mergeModuleCss(m) {
    var dir = m.path;
    log('merge[css] module path:', dir);
    return gulp.src([dir + '/*.scss'], {base: config.webappDir})
        .pipe(sass())
        // 去掉css注释
...
function mergeComponentsByModule() {
    ...
    modules.forEach(function (m) {
        ...
        gulp.src([dir + '/**/*.component.js', '!' + dir + '/*.all.component.js'], {base: config.webappDir})
...
function mergeModuleComponent(m) {
    var dir = m.path;
    log('merge[comp] module path:', dir);
    return gulp.src([dir + '/**/*.component.js'], {base: config.webappDir})
...
function mergeModuleService(m) {
    var dir = m.path;
    log('merge[serv] module path:', dir);
    return gulp.src([dir + '/**/*.service.js'], {base: config.webappDir})
        .pipe(plumber({errorHandler: handleErrors.reportError}))
        .pipe(concat(m.name + '.all.service.js'))
        .pipe(mapStream(function (file, cb) {
            _deleteFileInTmp(file.path, m.name);
...
function mergeAllStates() {
    ...
    return gulp.src(config.tmp + 'app/**/*.state.js')
...
function mergeStatesByModule() {
    ...
    return gulp.src(stateFiles)
        ...
        .pipe(concat('app.all.state.js'))
        .pipe(mapStream(function (file, cb) {
            var appState = config.tmp+'app/app.all.state.js';
...
function _deleteFileInTmp(_path, name) {
    var nPath = _path.split(path.sep).join('/').replace(config.webappDir, config.tmp + 'app/' + (name ? name + '/' : ''));
